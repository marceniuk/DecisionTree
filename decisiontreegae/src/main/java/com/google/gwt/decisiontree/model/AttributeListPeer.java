...
public class AttributeListPeer {
 public static Hashtable getAttributes(DataManager dataManager, String problemName) {
     ...
     */
	Problem problem = (Problem)(ObjectifyService.ofy().load().ancestor(Key.create(Problem.class, "ProblemName")));
    ...
    if (attributes.isEmpty()) {
	  ...
	  for (Attribute attribute : attributes) {
        ...
        attribute_list.put(attribute.id,attribute_for_list);
		}
	}
 ...
 public static Hashtable Get_splitting_outcomes (DataManager dataManager, String sAttributeFieldName, String problemName) {
    ...
    // Run an ancestor query 
	  List<Tuple> tuples = ObjectifyService.ofy()
          .load()
          .type(Tuple.class) // We want only Tuple
          .ancestor(theProblem)    // Anyone in this tuple
          .list();
	
	for(Tuple tuple: tuples) {
		Key theTuple = Key.create(Tuple.class, tuple.tuple_id);
		List<CategorizedData> categorizeddatas = ObjectifyService.ofy().load().type(CategorizedData.class).ancestor(theTuple).list();
		List<CategorizedData> categorizeddatas_sFieldName = new List<CategorizedData>;
		for(CategorizedData categorizeddata: categorizeddatas) {
			if(categorizeddata.theAttribute().get().attributeFieldName == sAttributeFieldName) {
				categorizeddatas_sFieldName.add(categorizeddata); // categorizeddatas_sFieldName - all the data of sFieldName
			}
		}
	}	
	}
